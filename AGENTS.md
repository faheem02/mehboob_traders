...
- Roles: `admin`, `order_booker`, `loader` (live logins; `salesman` remains an `employees.employee_type` value but salesman employees have NO login). Use `isAdmin()`, `isSalesTeam()` (= order_booker only), `salesManager(...)` helpers. Order Booker creates sales in `modules/sales/` and assigns a Salesman (`sales.salesman_id` → `employees.id`) to deliver; printed invoice shows "Delivered By".
